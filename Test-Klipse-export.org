...
#+BEGIN_SRC clojure :exports none
(defn break-segment [max-len-sq [p0 p1]]
(let[delta (add p1 (minus p0))
len-sq (dot-product delta)]
 (let[n (int (+ 0.5 (/ len-seq max-len-seq)))
      delta-i (scale (/ 1 n) delta)
      [last res]
(reduce (fn[[last res] i](let[next (add last delta-i)]
[next (conj res [last next])]))
[p0[]] (range (dec n)))]
(conj res [last p1]))));; we want the last point to be exact
#+END_SRC
